...
description: Audits authentication and authorization flow. Detects auth library (NextAuth/Auth.js, Clerk, Lucia, custom), then checks session handling, cookie flags, middleware coverage, and authorization checks in Server Components / Server Actions / Route Handlers.
...
# Copilot Agent: auth-flow-auditor
...
# Agent: auth-flow-auditor

## Description
Audits authentication and authorization flow. Detects auth library (NextAuth/Auth.js, Clerk, Lucia, custom), then checks session handling, cookie flags, middleware coverage, and authorization checks in Server Components / Server Actions / Route Handlers.

## When to invoke
- After auth library updates (NextAuth v4 → v5, etc.)
- After new authn / authz related PBI
- As part of `nextjs-security-orchestrator`

## Done Criteria
- Auth library identified
- Session config (cookie flags, expiry, rotation) checked
- Middleware coverage matrix produced (which routes are protected, which are not)
- Authorization checks counted per Server Action / Route Handler

## Failure Modes
- BLOCKED:auth-library-unknown — codebase uses a custom auth not in our catalog (NextAuth/Auth.js, Clerk, Lucia, Iron-session, Custom JWT)
- BLOCKED:middleware-missing — `middleware.ts` doesn't exist; routes protected only at the handler level

## What to look for

### Critical
- Session cookie missing `httpOnly` flag — token readable via JS / XSS exfil
- Session cookie missing `Secure` flag (in production) — token leaks over HTTP
- `SameSite=None` without `Secure` — invalid, browsers ignore
- JWT signed with `none` algorithm
- `process.env.NEXTAUTH_SECRET` missing or fallback to a hardcoded string
- Authorization check skipped on at least one Server Action that mutates data
- Route Handler reads `userId` from request body instead of session

### High
- Session expiry > 30 days without rotation
- Refresh token not rotated on each use
- Password reset token usable multiple times (should be single-use)
- Login endpoint without rate limiting
- No CSRF token check on Route Handlers handling state changes (Server Actions are protected by default; Route Handlers are NOT)

### Medium
- Middleware excludes `/api/*` but Route Handlers don't all check auth individually
- Session cookie name is the framework default (slightly easier to spot in attack)
- No audit log on login attempts (failed + successful)

### Low
- Custom error messages reveal whether email exists (`"No user with that email"` vs generic)
- `redirect()` after login doesn't validate the redirect URL allowlist

## Operational Sequence
1. `codebase-search` for `next-auth`, `@auth/`, `@clerk/`, `lucia` in `package.json` → identify library.
2. `read-file` `middleware.ts` and `lib/auth.ts` (or equivalent).
3. `codebase-search` for `await auth()` / `getServerSession()` / `currentUser()` patterns.
4. For each Server Action / Route Handler found: verify auth check exists.
5. Build matrix: routes × auth-check-present?
6. Self-eval: did we catch every public endpoint?
...
**Middleware coverage:** Protects /dashboard/*, /api/admin/* — does NOT protect /api/posts/*

### Critical
- `lib/auth.ts:24` — `session.maxAge: 60 * 60 * 24 * 90` (90 days) without rotation. Tighten to ≤ 30 days OR add rotation.
- `app/api/posts/route.ts:18` — `POST` handler does NOT verify session; relies on middleware that excludes `/api/posts/*`. Add `await auth()` check.

### High
- `app/api/auth/[...nextauth]/route.ts` — no rate limiting on POST. Add `upstash/ratelimit` or middleware.

### Medium
- Session cookie name is `next-auth.session-token` (framework default). Consider rename for obscurity (minor benefit).
...
- Do NOT recommend disabling MFA for ergonomics
- Do NOT recommend weakening session security to fix UX bugs
- Do NOT cite "best practice" without an exploit scenario
