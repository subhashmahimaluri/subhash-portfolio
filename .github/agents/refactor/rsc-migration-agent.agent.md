---
name: rsc-migration-agent
description: Migrates Client Components to Server Components where possible. Specifically targets components with `'use client'` that don't actually use client-only APIs — they can run on the server and remove themselves from the JS bundle.
...
# Copilot Agent: rsc-migration-agent
...
# Agent: rsc-migration-agent

## Description
Migrates Client Components to Server Components where possible. Specifically targets components with `'use client'` that don't actually use client-only APIs — they can run on the server and remove themselves from the JS bundle.

## When to invoke
- `pr-performance-reviewer` flagged unnecessary `'use client'`
- Bundle analysis shows a big tree shifted to client
- Manual PBI: "convert X to RSC"

## When NOT to invoke
- Component uses `useState` / `useEffect` / `useRef` / `useContext` / event handlers / `window` → it really needs `'use client'`
- Want to add a feature → wrong agent
- Want to split a component → `component-decomposer`

## Done Criteria
- Component file no longer starts with `'use client'`
- All `useState` / `useEffect` / `useRef` / `useContext` calls have been removed (because they weren't needed) OR proven to be unneeded and replaced with async data fetching
- Component is `async` if it now fetches data
- All existing tests still pass (no behavior change)
- No props deserialization issues — data is passed as plain objects, not functions/class instances
- Self-eval confirms behavior preserved + `'use client'` removed

## Failure Modes
- BLOCKED:client-api-actually-used — re-examination shows the component genuinely needs to be client
- BLOCKED:parent-passes-function-prop — parent passes a callback that would break when migrated; parent needs refactoring first
- BLOCKED:test-fails-after-migration — tests were over-asserting on client-render behavior

## Operational Sequence
1. `read-file` the target component.
2. List the client-only API usages (hooks, event handlers, window access).
3. For each:
   - Can it move to a server fetch? (e.g. `useEffect(() => fetch(...))` → `await fetch(...)` at top of component)
   - Can it move to a child Client Component? (e.g. a small `<ThemeToggle />` island, parent stays server)
   - Is it actually unneeded? (e.g. `useState` initialized once and never updated)
4. If all usages can be eliminated, plan the migration.
5. `apply-edit`: remove `'use client'`, convert to `async function`, replace `useEffect` data fetch with direct call.
6. `run-tests` — must all pass.
7. If any test fails, roll back the edit and re-evaluate.
8. `self-evaluate`: behavior unchanged? bundle smaller?

## Migration patterns

### Before (unnecessary client)
```tsx
'use client';
...
export function RecentPosts() {
  ...
  useEffect(() => {
    fetch('/api/posts/recent').then(r => r.json()).then(setPosts);
  }, []);
...
### Island pattern (parent stays server)
```tsx
// Before: whole page is 'use client' just for a small toggle
// After: only the toggle is client; rest stays server-rendered

// page.tsx — no 'use client'
import { ThemeToggle } from './ThemeToggle';   // small client island
export default async function Page() {
  const data = await loadData();
  return <main><ThemeToggle /><Body data={data} /></main>;
}
...
## Anti-patterns
- Do NOT remove `'use client'` if the component truly needs it — that breaks the build
- Do NOT change exported names or signatures — pure restructure
- Do NOT skip running tests
- Do NOT swap `fetch` for a server-only library that the component doesn't have access to in the bundle
