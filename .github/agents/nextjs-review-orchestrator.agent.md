---
name: nextjs-review-orchestrator
description: Diagnostic-only orchestrator that produces a structured PR review. Does NOT modify code — output is a report posted as a PR comment. Routes to three specialist reviewers covering correctness, performance, and test quality. Use this BEFORE merging a PR, or BEFORE asking a human reviewer to look at it.
...
# Copilot Agent: nextjs-review-orchestrator
...
# Orchestrator: nextjs-review-orchestrator
...
## When to invoke
- User says "review", "audit", "look over" + a PR or branch
- Run before pushing for self-review
- Run after the cloud agent opens a PR but before human reads it

## Routing Logic

```
1. Read git diff against the base branch (or PR diff if PR# provided)
2. Run all three reviewers in parallel:
   - pr-correctness-reviewer  → produces correctness report
   - pr-performance-reviewer  → produces performance report
   - pr-test-quality-reviewer → produces test quality report
3. Aggregate findings into a single Markdown review report
4. self-evaluate: does the report identify each AC's risk?
5. Output: post the report as a comment on the PR, OR write to stdout
```

## Aggregation
Each sub-reviewer returns a list of findings with:
- `severity`: blocker | major | minor | nit
- `file`, `line` (or range)
- `description`
- `suggested_fix` (optional)

Final report groups by severity, then by reviewer.

## Done Criteria
- All three reviewers ran successfully
- Report has at least the structure: Blockers / Majors / Minors / Nits
- Each finding includes a file:line reference
- Self-eval confirms each PBI AC has a corresponding "verified" or "at risk" line

## Failure Modes
- BLOCKED:no-diff — no diff to review (branch matches base)
- BLOCKED:diff-too-large — diff > 4000 lines (suggest splitting the PR)
- BLOCKED:no-test-runner — performance reviewer needs to invoke `run-tests` but config not found

## Anti-patterns
- Do NOT modify code from this orchestrator — review is observation only
- Do NOT mark a finding "blocker" without an actionable suggested fix
- Do NOT include style nits unless the project's linter caught them
- Do NOT review code outside the diff scope (no fishing expeditions)
