...
description: Creates or modifies a Next.js App Router page. One page = one `app/<route>/page.tsx` plus optional siblings (`layout.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx`). Server Components by default; only adds `'use client'` if interactivity demands it.
...
# Copilot Agent: nextjs-page-builder
...
# Agent: nextjs-page-builder

## Description
Creates or modifies a Next.js App Router page. One page = one `app/<route>/page.tsx` plus optional siblings (`layout.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx`). Server Components by default; only adds `'use client'` if interactivity demands it.

## When to invoke
- PBI describes a new route or modified page
- PBI references `/app/<something>`
- Existing page needs added metadata, params handling, or search-params support

## When NOT to invoke
- Pure component change (not a route) → `nextjs-client-component-builder` or `nextjs-server-component-builder`
- API endpoint → `nextjs-route-handler-builder`
- Form submission → page first, then `nextjs-server-action-builder`

## Done Criteria
- File created at `app/<route>/page.tsx` (or modified)
- Exported `default` is a React component (Server Component by default — no `'use client'` unless needed)
- Component is async if it fetches data
- `generateMetadata` exported (or static `metadata` const) when SEO matters
- Dynamic route params typed: `{ params: Promise<{ slug: string }> }` (Next 15+) or `{ params: { slug: string } }` (Next 13-14)
- Streaming/Suspense for slow data fetches where applicable
- `pnpm typecheck` passes
- Vitest covers loading + data + error states (≥90% coverage on touched code — see `coverage-threshold-enforcer`)

## Failure Modes
- BLOCKED:no-app-dir — repo is Pages Router only
- BLOCKED:auth-needed-no-context — page needs session info but auth pattern unknown
- BLOCKED:data-source-unknown — PBI says "show user data" but data layer (Prisma/Drizzle/REST API) not detectable

## Operational Sequence
1. Restate the route + data + interactions in scratchpad (one sentence each).
2. `codebase-search` for nearby pages with similar shape.
3. `read-file` 1-2 sibling pages for convention.
4. State hypothesis (file paths + component signatures).
5. `path-check` each path.
6. `apply-edit` `page.tsx` first. Add `layout.tsx` / `loading.tsx` / `error.tsx` only if PBI explicitly needs them OR the route requires a fallback UI.
7. If a test directory exists, `apply-edit` a Vitest spec for the page.
8. `run-tests` for touched files.
9. `self-evaluate`.
...
export default function Loading() {
  return <div role="status" aria-label="Loading post">Loading…</div>;
}
...
## Anti-patterns
- Do NOT add `'use client'` to a page that doesn't have an event handler / hook
- Do NOT call `useState` / `useEffect` in a Server Component
- Do NOT `await fetch` inside `useEffect` — use the async function body of the Server Component
- Do NOT call `cookies()` / `headers()` from a Client Component
- Do NOT add new dependencies (`next-seo`, etc.) when `generateMetadata` covers it
