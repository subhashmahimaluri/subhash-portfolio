---
name: nextjs-server-component-builder
description: Implements a React Server Component (RSC) — the default in Next.js App Router. RSCs run only on the server, can be async, can directly call DB/ORM/internal APIs, and cannot use browser-only APIs (`useState`, `useEffect`, `onClick`, `window`).
...
# Copilot Agent: nextjs-server-component-builder
...
# Agent: nextjs-server-component-builder

## Description
Implements a React Server Component (RSC) — the default in Next.js App Router. RSCs run only on the server, can be async, can directly call DB/ORM/internal APIs, and cannot use browser-only APIs (`useState`, `useEffect`, `onClick`, `window`).

## When to invoke
- Component fetches data from DB/ORM/REST API
- Component composes other Server Components
- Component shows server-only secrets (admin badges, env-based feature flags)
- PBI describes a "display" or "read-only" view

## When NOT to invoke
- Component needs interactivity (clicks, form input, hover state) → `nextjs-client-component-builder`
- Component needs browser APIs (localStorage, geolocation, IntersectionObserver) → client
- Component IS a page → `nextjs-page-builder`

## Done Criteria
- File does NOT start with `'use client'`
- Component is `async function` if it fetches data
- Data fetched directly (`await db.query(...)`, `await fetch(...)`) — no SWR/React Query needed for one-shot fetches
- Props are typed; no `any`
- `cache()` from `react` used when the same data is fetched from multiple components in one render
- `pnpm typecheck` passes
- Vitest covers happy path + empty/error data state (≥90% coverage)

## Failure Modes
- BLOCKED:client-api-leak — component implementation requires a hook or DOM API
- BLOCKED:data-source-circular — fetching pattern creates a loop with another RSC
- BLOCKED:not-rsc-eligible — component needs Suspense boundaries that don't exist yet

## Operational Sequence
1. Restate the component's purpose in scratchpad: "Renders X using server-fetched Y."
2. `codebase-search` for the data source pattern (`db.posts.findMany`, `fetch('/api/...')`, etc.).
3. `read-file` 1-2 sibling Server Components.
4. Identify any computed values that should be cached with `React.cache()`.
5. State hypothesis: file path + props interface + data fetching call.
6. `apply-edit` the component file.
7. `apply-edit` a Vitest test (mock the data fetcher, assert render).
8. `run-tests`.
9. `self-evaluate`.
...
```tsx
// app/components/RecentPosts.tsx — no 'use client'
import { db } from '@/lib/db';

type Props = { limit?: number };

export async function RecentPosts({ limit = 5 }: Props) {
...
### Cached fetch shared across components
```tsx
...
// Each component that calls getUserById('123') in the same render shares one DB call.
```

## Anti-patterns
- Do NOT add `'use client'` "just in case"
- Do NOT install SWR / React Query / TanStack Query for one-shot fetches — RSC handles it
- Do NOT use `useEffect` to fetch data in an RSC (it doesn't run on the server)
- Do NOT pass functions or class instances as props from RSC to Client Component — only serializable values
- Do NOT call DB queries inside a render loop without `React.cache()`
